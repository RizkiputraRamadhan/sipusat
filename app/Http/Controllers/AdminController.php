...
class AdminController extends Controller
{
    ...
    public function index(){
        ...
        $category = Oex_category::get()->count();
    ...
    public function add_new_category(Request $request){
        ...
        if($validator->fails()){
            ...
            $cat->status = 1;
            $cat->save();
            $arr=array('status'=>'true','message'=>'Kategori Berhasil Dibuat','reload'=>url('admin/catagory_sipusat'));
        }
    ...
    public function delete_category($id){
        ...
        $cat->delete();
        return redirect(url('admin/catagory_sipusat'));
    }
    ...
    public function edit_new_category(Request $request){
        ...
        $cat->name = $request->name;
        $cat->update();
        echo json_encode(array('status'=>'true','message'=>'updated successfully','reload'=>url('admin/catagory_sipusat')));
    }
    ...
    public function add_new_exam(Request $request){
            ...
            if($validator->fails()){
                ...
                $arr = array('status'=>'true','message'=>'Ujian Berhasil Dibuat','reload'=>url('admin/create_ujian_sipusat'));
    ...
    public function delete_exam($id){
        $exam1 = Oex_exam_master::where('id',$id)->get()->first();
        $exam1->delete();
        return redirect(url('admin/create_ujian_sipusat'));
    }
    ...
    public function edit_exam_sub(Request $request){
        ...
        echo json_encode(array('status'=>'true','message'=>'Ujian Berhasil diperbarui','reload'=>url('admin/create_ujian_sipusat')));
    ...
    public function add_new_students(Request $request){
        ...
        if($validator->fails()){
            ...
            $arr = array('status'=>'true','message'=>'student added successfully','reload'=>url('admin/data_siswa_sipusat'));
        }
    ...
    public function delete_students($id){
        ...
        $std = user_exam::where('id',$id)->get()->first();
        $std->delete();
        return redirect('admin/result_sipusat');
    }
    ...
    public function delete_registered_students($id){
        ...
        $std->delete();
        return redirect('admin/data_siswa_sipusat');
    ...
    public function add_new_question(Request $request){
        ...
        if($validator->fails()){
            ...
            }elseif($request->ans=='option_4'){
                $q->ans=$request->option_4;
            }else{
                $q->ans=$request->option_5;
            }
            


            $q->status=1;
            $q->options=json_encode(array('option1'=>$request->option_1,'option2'=>$request->option_2,'option3'=>$request->option_3,'option4'=>$request->option_4,'option5'=>$request->option_5));
